...
In addition to the capture tank, a second plastic bin is in place as a cover, because the pressurized electrolyte injection causes a lot of overspray (which is corrosive to the leadscrews & rails). This worked fairly well, although the minimal overspray really calls for some rubber accordion boots to give a second layer of rust protection. I used a large glass beaker as a secondary holding tank to experiment with electrolyte filtration, and as a way to more easily replace the solution mid-cut if necessary without jostling the machine.
